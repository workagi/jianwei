...
- 每个计划时间点生成稳定幂等键，重复领取只会恢复同一次运行，不会创建重复账单。
- 外部采集和模型处理完成后，再用短事务一次提交内容、匹配、用量、游标和运行结果。
- 模型 RPM 由数据库统一排队，多台 Worker 共享同一服务商额度时也不会叠加超速。
- 付费请求先原子预留预算，成功后转为用量账单，失败或超时会释放占位。
- 暂时性错误进入短周期错峰重试。
- 连续失败达到阈值后可自动停用。
...
| `usage_reservations` | 外部请求开始前的预算占位、结算和失败释放状态 |
| `rate_limit_slots` | 多进程共享的模型请求时间槽 |
