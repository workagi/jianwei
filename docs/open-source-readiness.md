...
高级用户可以通过 `WERSS_IMAGE` 使用自行审核和构建的镜像。需要 Chromium 的全文能力由可选增强通道承担，不再要求所有用户拥有本机定制镜像。
...
- [ ] 确认信息流截图、演示数据和 README 不含真实账号或私人内容。
- [ ] 仓库切换为 Public 后立即启用 Private vulnerability reporting。
- [ ] 确认 `NOTICE` 中使用 `skymao2021` 作为当前权利人标识。
- [ ] 确认公众号与 X 功能说明没有暗示官方合作或永久稳定。
- [ ] 创建公开 Release，并说明实验性采集路径和已知限制。
- [ ] 由仓库所有者明确批准将仓库可见性切换为 Public。
