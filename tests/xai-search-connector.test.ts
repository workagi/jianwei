...
describe("XaiSearchConnector", () => {
  it("ingests only citation-backed posts from the requested account", async () => {
    const connector = new XaiSearchConnector(async () => "oauth-token", async () => response({
    ...
    const preview = await connector.validate({ provider: "x_grok", username: "OpenAI", includeReplies: false, includeReposts: false });
  ...
  it("retries transient xAI failures before giving up", async () => {
    let attempts = 0;
    const connector = new XaiSearchConnector(async () => "oauth-token", async () => {
      attempts += 1;
