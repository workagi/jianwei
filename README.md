...
第一次打开见微，信息流是空的。别急，按下面步骤来，5 分钟就能看到第一条内容。
...
**第 2 步：告诉见微你想关注什么**  
左侧菜单点「监控任务」→「添加监控」。你会看到几种信息来源：

| 我想关注… | 选这个 | 需要准备什么 |
| --------- | ------ | ------------ |
| 某个 X（Twitter）博主 | X 账号 | 什么都不用（默认走 SuperGrok） |
| 某个微信公众号 | 微信公众号 | 先配好 WeRSS（见下方第 3 步） |
| 某个关键词的全网动态 | 关键词搜索 | 配置 Brave Search API Key |
| 国内热榜、某个 RSS | 榜单 / RSS | 什么都不用 |

填好名称和关键词（可选），点「添加」就行。系统会自动开始采集。

**第 3 步（仅公众号需要）：配好 WeRSS**  
- 浏览器打开 http://localhost:8001 ，同样用 admin / admin@123 登录  
- 左侧「授权管理」→ 扫码绑定你的微信  
- 左侧「Access Key」→ 创建一个 AK，复制 AK:SK  
- 把 AK:SK 填到 .env 的 WERSS_ACCESS_KEY= 后面  
- 回到见微后台 →「平台连接」→ 确认 WeRSS 状态正常

**第 4 步：等着看结果**  
添加监控后，稍等 1-2 分钟，刷新信息流首页 http://localhost:3000 ，第一条内容就出现了。每条内容会告诉你「为什么值得看」。

> 不熟悉面板？看这张图：左侧菜单包括「信息流」（看结果）、「监控任务」（管理关注）、「平台连接」（配 API/公众号/模型）。大部分时间你只需要前两个。
