      ...
      const returned = await database
        ...
        .onConflictDoUpdate({
          ...
          set: {
            ...
            authorId: sql`coalesce(excluded."author_id", "author_id")`,
            authorName: sql`coalesce(excluded."author_name", "author_name")`,
            authorHandle: sql`coalesce(excluded."author_handle", "author_handle")`,
            avatarUrl: sql`coalesce(excluded."avatar_url", "avatar_url")`,
      ...
      const result = await database
        ...
        .onConflictDoUpdate({
          ...
          set: {
            ...
            relevanceScore: sql`coalesce(excluded."relevance_score", "relevance_score")`,
            retentionReason: sql`coalesce(excluded."retention_reason", "retention_reason")`,
            retentionSource: sql`coalesce(excluded."retention_source", "retention_source")`,
            rawPayload: sql`excluded."raw_payload"`,
            lastSeenAt: new Date(),
          },
        })
      ...
      const result = await database
        ...
        .onConflictDoUpdate({
          ...
          set: {
            ...
            status: sql`'claimed'`,
            claimedAt: sql`now()`,
            expiresAt: sql`now() + interval '${String(input.leaseMinutes)} minutes'`,
          },
          where: and(
